...
if (isset($_POST['login'])) {
    ...
    if (empty($error)) {
        $stmt = $conn->prepare("SELECT employee_id, password_hash, user_role, user_status FROM users WHERE username = ?");
        $stmt->bind_param("s", $username);
        $stmt->execute();
        $stmt->store_result();

        if ($stmt->num_rows > 0) {
            $stmt->bind_result($employee_id, $password_hash, $user_role, $user_status);
            $stmt->fetch();

            // Check if the account is deactivated
            if ($user_status === 'deactivated') {
                $error['account'] = 'Your account has been deactivated by the admin.';
